...
class CLIPVAD(nn.Module):
    ...
    def compute_and_add_difference(self, x):
        # 检查输入张量的维度是否正确
        if len(x.size()) != 3:
            raise ValueError("Input tensor must have 3 dimensions (batch_size, seq_len, dim)")

        # 获取序列的长度和维度大小
        batch_size, seq_len, dim = x.size()

        # 初始化一个张量来存储差值
        diffs = torch.zeros(batch_size, seq_len - 1, dim).to('cuda')
        ...
        x_new[:, 0:seq_len - 1, :] = x_new[:, 0:seq_len - 1, :] + diffs

        x_new = self.mlp2(x_new)
        x_new = torch.mean(x_new, dim=-1, keepdim=True)
        x_new = torch.sigmoid(x_new)
        return x_new


    def adj4(self, x, seq_len):
        soft = nn.Softmax(1)
        x = x + x*self.compute_and_add_difference(x)
    ...
    def forward(self, visual, padding_mask, text, lengths):
        ...
        text_features = text_features.expand(visual_features.shape[0], text_features.shape[1], text_features.shape[2])
        text_features_norm = text_features / text_features.norm(dim=-1, keepdim=True)
        text_features_norm = text_features_norm.permute(0, 2, 1)
        # logits2 = visual_features_norm @ text_features_norm.type(visual_features_norm.dtype) / 0.009
        logits2 = visual_features_norm @ text_features_norm.type(visual_features_norm.dtype) / 0.009
        # 取出片段最可能的异常类别
        element_logits2 = torch.sigmoid(logits2)
        # element_logits2 = logits2.softmax(dim=-1)
        selected_text_features = element_logits2 @ text_features
        selected_text_features_norm = selected_text_features / selected_text_features.norm(dim=-1, keepdim=True)
        visual_features = visual_features + selected_text_features_norm
